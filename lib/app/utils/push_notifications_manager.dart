...
class PushNotificationsManager {
  ...
  Future<StyleInformation?> getNotificationStyle(String? image) async {
    if ((image ?? "").isNotEmpty) {
      ...
      return BigPictureStyleInformation(ByteArrayAndroidBitmap(imageData.buffer.asUint8List()));
    } else {
      return null;
    }
  }
  ...
  void showNotification(String title, String body, String? payload, String? image) async {
    ...
    var androidPlatformChannelSpecifics = AndroidNotificationDetails('${Random().nextDouble()}', 'Fluuky Notification',
        importance: Importance.max, priority: Priority.max, playSound: true, styleInformation: notificationStyle);
  ...
  subscribeToTopic() {
    if (Platform.isIOS) {
      _firebaseMessaging.subscribeToTopic("Fluuky");
    } else {
      _firebaseMessaging.subscribeToTopic("Fluuky");
    }
  }
